...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        touched_area = state[1]
        return len(touched_area) == 4
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                if node in self.corners:
                    if node not in touched_area:
                        touched_area.append(node)

                touched_area = tuple(touched_area)
                successors.append(((node, touched_area), action, 1))
